...
Run_RSTAN_ToxKin_mod <- function(stan_mod,datafile,var_y,var_z,var_t,parms_out,priors,t_e=t_d,var_grp="phase_no",
  ...
  rs.dat <- list(N=nrow(datafile), z=mean(datafile[,var_z]), t_d=t_e, y=datafile[,var_y], x=datafile[,var_t], grp=datafile[,var_grp], no_sigma=n_sigma,
  ...
  print(paste0(var_y," data from experiment initialized, RSTAN runs...")) # Notify user that RSTAN simulation starts
...
plot_RSTAN_res <- function(x_rstan, header, hsk=FALSE, y_lim=3, x_lim=c(0,100), y_ij, y_lab='Residuals', x_lab="", phase=NULL){
  ...
  if (max(abs(res)) > 3) y_lim <- max(abs(res)) # Check if standardized residuals are more than 3SD away from zero and extend y-axis range accordingly
...
Plot_PostPredDistr <- function(summ_mod01, summ_mod02, raw_data, var_x, var_y, title, y_lim=c(-15,345), main01="Homoscedastic", main02="Heteroscedastic"){
  ...
  t_xyz <- raw_data$Time #  Extract time points for observations in the data
  C_exp <- mean(raw_data$C_exp) # Compute mean of the toxicant concentration in the exposure medium
  ...
  # Paste outer margin axis lables
  mtext(list("Time",title), 1:2, cex=1.25, outer=T, line=c(0.25,1.0)) 
} # End plot Plot_PostPredDistr()


# The compute_loo() function computes model evaluation metrics for a Bayesian model objects fitted using RSTAN.
# It calculates the Leave-One-Out Information Criterion (LOOIC) for the RSTAN model fit, It extracts the log-likelihood from the RSTAN
# objects, calculates the WAIC, converts the log-likelihood to an array format and computes the relative efficiency, then
# computes the LOOIC using the extracted log-likelihoods and relative efficiency and returns the LOOIC value.
# This function is useful for model selection and comparison in Bayesian analysis, where lower LOOIC values indicate better model fit.
# The function has only one arguments: a Bayesian model object fitted using RSTAN. 
